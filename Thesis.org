...
*** If statement
The most trivial way to switch between one of two steps is to use an ~if~ statement. While not offering composability, a the simple ~if~ statement is highly effective for binary choices. An example can be seen in listing 8, where we implement an guard for the second step shown in figure 2. The first step, ~login-information~ (defined in listing 4), is shown until it emits it's inner state.
...
** TODO Why monads are a good fit
