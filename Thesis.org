...
- Early termination; Seeing how the use-case is all about making it possible to compose steps together, asking if it even should be possible to premeaturely exit the chain of operation, from within a ~Prog~, is valid, however it's important to keep in mind that a ~Prog~ could be used from different parts of the application and the developer would have no idea if that ~Prog~ has the possibility to terminate itself (and therefore the chain) early, without looking at the code.
- Different types of commit
- map actions to commit (from outside)
- helper functions like ~def-prog~
