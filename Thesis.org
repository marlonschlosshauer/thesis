...
*** Custom TCO
While ~loop~ provides a perfectly fine way to get the benefits of TCO for synchronise functions, in order to work with the asynchronous, action driven approach that reacl-c uses, a custom implementation needed to developed. We will revisit the trampoline again during the implementation.
* Implementation
...
*** TODO Implementing Tail Call Optimization
