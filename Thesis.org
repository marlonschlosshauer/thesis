...
Before we dive deeper into the unique features that reacl-c has to offer, we'd like to give an overview of the most important functions. Take a look at table 1 to see a brief overview of some of the functions which we will use to build our components. We will explain them further in the comming chapters.

| Name            | Signature                            | Description                                                                                              |
| ~local-state~   | ~x -> Item -> Item~                  | Makes the first parameter available as inner state inside of the second parameter.                       |
| ~dynamic~       | ~([o, i]) => Item -> Item~           | Gives access to (outer & inner) state inside of the function. The function needs to return an ~Item~.    |
| ~handle-action~ | ~Item, ([o, i]) => Returned -> Item~ | Catches ~actions~ emitted by the first parameter. Calls the second parameter when an ~action~ is caught. |
| ~return~        | ~:action -> x -> Returned~           | Returns something that can be consumed to either trigger an emit of an action or a state change.         |
| ~focus~         | ~Lens -> Item -> Item~               | Restrict state of the second parameter to only what the first parameter (lens) allows.                   |
| ~div~           | ~[Item] -> Item~                     | Bundle multiple ~Items~ into one ~Item~ (just like a ~<div>~ would do in HTML).                          |
...
Having understood the most important functions, we're now ready to learn how to use these basic building blocks. Let's begin by talking about how we can use and define ~Items~.
*** How an Item is made
...
#+CAPTION: Showcasing composition of multiple ~items~ into one by using a ~div~, by building the a static version of the first step shown in figure 2.
#+begin_src clojure -n 1
  (dom/div
   (dom/img {:src "./img/logo.png"})
...
Let's take a closer look at how we can work with state in reacl-c. Afterwards we will examining how we can make our components interactive.
*** State Management in reacl-c
Like with React, handling state is very important in reacl-c. Thankfully the framework gives the developer many ways to tackle this problem. Also like React, developers can easily make out if a component is using or changing state, which makes working with state easier.
...
We can see the previously mentioned ~core/local-state~ (line 4-6) to add a map with two fields as inner state. The second argument is an ~Item~ which the inner state will be applied to. But, just ~core/local-state~ isn't enough, as we don't have a keyword to access our inner state.
To access that newly added inner state, we need to use the ~core/dynamic~ function. The function itself takes a single parameter: Another function, that now must return an ~Item~. That function gets the surrounding state as an argument (line 8), therefor anything inside of it has at least read access to the state. In listing 4 the parameter of that function has been destructured into outer and inner state (line 8). We can see that the function will return a ~core/div~ and that the provided inner state is used in lines 13-14 & 18-19.

But we can do more than just access state. With the ~core/return~ function are able to change state (and send actions, see next chapter). We can see usage of ~core/return~ in listing 4 at lines 13, 18 & 20. The function takes two arguments. A keyword and a value. If the ~:state~ keyword has been supplied (like in line 13, 18), we will set the state to the second argument, the value. This will change both inner and the inheritet outer state. In line 13 we can see that the ~:email~ keyword in the inner state is to be updated to the value of the event while the outer state is kept the same. Outer and inner state are usually supplied through a single list, which is why we also enter a list as our value which the state will be set to.

Now we know how to both access and update state. This is an important feature that reacl-c provides. But it isn't the only important feature. Next we're going to look at how components can interact with each other by using ~core/handle-actions~ and ~core/return~.
*** Emitting and handling actions
Both ~core/handle-actions~ and ~core/return~ work by using actions. These Actions are an important tool for children to communicate with their parents. Like the implicit (or explicit) passing of state from the parent, this is a crucial feature to make ~Items~ composable.
To emit an action the child needs to call ~core/return~ with the ~:action~ keyword and the desired payload.
This can be seen in listing 4 in line 20, as a result of user input (button pressed). The ~core/return~ function can also be seen using the ~:state~ keyword in line 13 and 18, to change the state of the component. More on that soon.
The emitted action from the button press will then travel upwards until it is caught by a ~core/handle-action~. Like shown in table 1, ~core/handle-action~ takes two parameters, an ~Item~ which to wrap and a ~function~, which will be called once an action has been caught.

Take a look at listing 5 to see our previously in listing 4 defined ~login-information~ component being wrapped by a ~core/handle-action~. First we define some local state (line 2-3) to keep track if the button has been pressed. Then inside of that local state, we call ~core/handle-action~ (line 4-11) and give it both a an ~Item~ (in the form of a ~core/dynamic~) and a function (line 10-11). In line 5 we call ~core/dynamic~,  which takes a function that returns an ~Item~. The function gets the state of the component as a parameter, therefor has now at least read access to the state. We destructure the
    ...
    (core/local-state
     ...
     (core/handle-action
      (core/dynamic
       (fn [[_ inner]]
	 (if (:pressed? inner)
	   (dom/h2 "Button pressed, show next step!")
	   (login-information))))
      (fn [[outer inner] ac]
	(core/return :state [outer {:pressed? ac}])))))
#+end_src
** What is a monad
