...
Macros can be found all over Clojure and ClojureScript. Functions like ~when~[7] and ~when-not~[8] are actually macros that rewrite themselves to a simple ~if~. Even the much discussed ~let~ function is a macro[9].
*** Syntax
...
If we quote something with ~\'~ or ~\`~ it will not be evaluated yet[10]. The difference between the two is that ~\`~ qualifies each expression with their full namespace. The ~\~~ symbol turns on evaluation inside of a ~\`~ again[10].
...
This allows us to avoid having to flatten the ~Bind~ anywhere else, which makes showing the ~Item~ inside of the ~Bind~ (and ~Prog~) trivial. It also guarantees that the order of execution will always be correct, thanks to deconstructing the passed ~Bind~ completely.
...
- [7] https://github.com/clojure/clojure/blob/clojure-1.10.1/src/clj/clojure/core.clj#L493
- [8] https://github.com/clojure/clojure/blob/clojure-1.10.1/src/clj/clojure/core.clj#L499
- [9] https://github.com/clojure/clojure/blob/clojure-1.10.1/src/clj/clojure/core.clj#L4481
- [10] Quick Clojure: Effective Functional Programming - 10. Macros
- [11] Tail Call Optimization in the Java HotSpotTM VM
- [12] https://2ality.com/2015/06/tail-call-optimization.html
- [13] https://www.learn-clojurescript.com/section-2/lesson-11-looping/
